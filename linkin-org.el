...
The id format is YYYYMMDDTHHmmSS (year, month, day, T, hour, minute, second)."
...
;;;###autoload
(defun linkin-org-open-link-at-point-in-dired ()
...
(defun linkin-org-file-open (link)
  ...
  (let* ((file-path (org-element-property :path link))
     ...
     (when (file-exists-p file-path)
	   ...
	   (linkin-org-open-path-in-dired link)
...
Otherwise, calls the function STD-LINK-OPENING-FUNCTION to open it."
  (let (;; resolve the link, so that it has a correct path and metadata
    ...
    (cond
     ;; if we want to open the link in dired
     (linkin-org-open-in-dired-p
      (linkin-org-open-link-in-dired link)
      )
     ;; else, if the link has type file and we want to open it as if it was opened from dired
     (
      (and linkin-org-open-links-as-in-dired-p
