...
- *reliable*: your links just always work.
  It takes a bit of discipline (see the [[#how-reliable-are-the-links-exactly][4 golden rules]]); also, you should not be afraid of seeing ids everywhere.
...
Also, it suggests a simple, blazingly fast workflow to store and retrieve your data: store the data (~linkin-org-store~), paste the link in your note, retrieve the data (~linkin-org-follow~).
...
2. *Make sure that each id is unique.*
   ...
   But be careful when you start copying files: if you dont change the file name of the copied file, you end up with two simimlar ids.
...
- Place the most relevant link at the top of the note.
  ...
  With this, your fingers quickly learn the automation "go to the note -> org-next-link -> linkin-org-follow" to access a data quickly.
