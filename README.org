...
linkin-org --- an emacs workflow with simple, fast, and reliable links.
...
It works on a file (in a dired buffer), a line in an editable file (org, markdown, python, whatever), selected words in a pdf (in [[https://github.com/vedang/pdf-tools][pdf-tools]]), a mail attachment (in mu4e) ...
...
linkin-org proposes to store precise locations with links.
...
You should now redefine ~linkin-org-store~ by adding the following code in the ~cond~ environment.
...
You should redefine ~linkin-org-store~ by adding the following code in the ~cond~ environment.
Replace <your link> by a link towards the location where you want to save the text.
To obtain such a link, go at the place where you want to store your saved texts and run ~linkin-org-store~.
#+begin_src emacs-lisp
;; If in elfeed
((string= mode "elfeed-search-mode")
 (my-save-elfeed-entry "<put your link there>")
 )
#+end_src
