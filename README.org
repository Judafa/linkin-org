...
- *reliable*: your links just always work.
  ...
  Give it a try! It's nothing a little habit cannot handle.
- *fully distributed*: rename, move, modify your files from literally anywhere without breaking your links.
  All you have to do is to not change or modify the ids in the file names.
- *non-intrusive*: you can store your files with other methods in parallel (directory hierarchy, file-naming, etc) without problem.
  The only difference is, the files that are linked have an id in their names.
...
Also, it suggests a new simple, blazingly fast workflow to store and retrieve your data: store the data (~linkin-org-store~), paste the link in your note, retrieve the data (~linkin-org-follow~).
...
An id system makes sure that your links will always work even if you move, rename or modify the linked data (see the [[#how-reliable-are-the-links-exactly][3 golden rules]]).
