...
From a plain-text file, follow a link that you previously pasted there to access a precise location: a file, a given place in a text file, precise words in a pdf.
The links are fast and easy to create; most importantly the links are *reliable*, which makes it possible to base your workflow on them.
