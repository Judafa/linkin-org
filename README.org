...
- ~linkin-org-store~ stores what you have at point so that links towards it become *reliable*.
  ...
  The link is reliable thanks to an automated id system: with linkin-org, a file that you can reliably access has a name like:
  #+begin_src example
  20250425T155433--filename.txt
  #+end_src
  You can rename your file (do not modify the id of course!), move, or modify the file, the link still works thanks to the id.
