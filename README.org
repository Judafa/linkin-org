...
linkin-org proposes to access any data with reliable links.
It suggests a simple, crazy effective worflow centered around your written notes.
linkin-org is:
...
- *non-intrusive*: you can organize your files with other methods in parallel (directory hierarchy, file-naming, etc) without problem.
  The only noticeable difference is, the files that are reliably linked have an id in their names.
- *simple*: linkin-org introduces two additional commands, ~linkin-org-store~ and ~linkin-org-get~


linkin-org turns your written notes into precise, reliable access points towards your data.
Also, it suggests a simple, blazingly fast workflow to store and retrieve your data: store the data (~linkin-org-store~), paste the link in your note, retrieve the data (using the standard org link opening command ~org-open-at-point-global~).
...
(use-package linkin-org
  ...
  :config
  ;; change this default to put stored files where you want them
  (setq linkin-org-store-directory "~/")
  ;; this is a list of directories that will be searched (in order) to resolve broken links
  ;; this is only done if your linked data has an id attached
  ...
  ;; uncomment this to open your linked files as if you opened them from dired
  ;; (setq linkin-org-open-links-as-in-dired-p t)
  ;; (setq linkin-org-opening-file-function #'dired-open-file)
